...
let CorrelationIdMiddleware = class CorrelationIdMiddleware {
    use(request, response, next) {
        const correlationId = (0, observability_1.ensureCorrelationId)(request.header(observability_1.CORRELATION_HEADER));
        request.correlationId = correlationId;
        response.setHeader(observability_1.CORRELATION_HEADER, correlationId);
        (0, observability_1.withCorrelationContext)(correlationId, () => next());
    }
};
